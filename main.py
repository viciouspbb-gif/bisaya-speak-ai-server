...
# 最小必要バージョン
MINIMUM_APP_VERSION = "1.0.9"


def check_app_version(app_version: Optional[str]) -> bool:
    """
    アプリバージョンをチェック
    
    Parameters:
    - app_version: アプリのバージョン（例: "1.0.9"）
    
    Returns:
    - bool: バージョンが要件を満たしているか
    """
    if not app_version:
        return False
    
    try:
        # バージョン文字列を数値のリストに変換（例: "1.0.9" -> [1, 0, 9]）
        app_parts = [int(x) for x in app_version.split('.')]
        min_parts = [int(x) for x in MINIMUM_APP_VERSION.split('.')]
        
        # バージョンを比較
        for i in range(max(len(app_parts), len(min_parts))):
            app_num = app_parts[i] if i < len(app_parts) else 0
            min_num = min_parts[i] if i < len(min_parts) else 0
            
            if app_num > min_num:
                return True
            elif app_num < min_num:
                return False
...
async def check_version(app_version: Optional[str] = Header(None, alias="X-App-Version")):
    """
    アプリバージョンチェック専用エンドポイント
    
    Parameters:
    - app_version: アプリのバージョン（ヘッダー: X-App-Version）
    
    Returns:
    - バージョン互換性情報
    """
    is_compatible = check_app_version(app_version)
    
    return {
        "compatible": is_compatible,
        "server_version": "1.0.9",
        "minimum_app_version": MINIMUM_APP_VERSION,
        "current_app_version": app_version or "unknown",
        "message": "アプリは最新です" if is_compatible else f"アプリを{MINIMUM_APP_VERSION}以上にアップデートしてください"
    }
...
async def check_pronunciation(
    ...
    """
    # アプリバージョンチェック
    if not check_app_version(app_version):
        raise HTTPException(
            status_code=426,
            detail={
                "error": "app_update_required",
                "message": f"アプリのバージョンが古いため、使用できません。最新バージョン（{MINIMUM_APP_VERSION}以上）にアップデートしてください。",
                "minimum_version": MINIMUM_APP_VERSION,
                "current_version": app_version or "unknown"
            }
        )
        ...
        if word:
            ...
            if reference_path.exists():
                ...
                try:
                    ...
                    print(f"フィードバック: rating={feedback.get('rating')}, details数={len(feedback.get('details', []))}")
                except Exception as e:
                    import traceback
                    print(f"音声処理エラー: {e}")
                    ...
                    # エラー時のダミーcomparison
                    comparison = {
                        "user_features": {"duration": 0, "pitch_mean": 0, "pitch_std": 0},
                        "reference_features": {"duration": 0, "pitch_mean": 0, "pitch_std": 0}
                    }
                
                # レスポンスデータを作成
                user_features = comparison.get('user_features', {"duration": 0, "pitch_mean": 0, "pitch_std": 0})
                reference_features = comparison.get('reference_features', {"duration": 0, "pitch_mean": 0, "pitch_std": 0})
                
                # デバッグログ
                print(f"=== 比較詳細データ ===")
                print(f"ユーザー特徴量: {user_features}")
                print(f"参照特徴量: {reference_features}")
                print(f"====================")
                ...
                print(f"レスポンス送信: pronunciation_score={response_data['pronunciation_score']}")
